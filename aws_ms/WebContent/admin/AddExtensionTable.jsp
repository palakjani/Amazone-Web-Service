        ...
        <div class="panel-body">
          <div class="table-responsive">
            <table id="dataTable1" class="table table-bordered table-striped-col">
              ...
              <c:forEach items="${sessionScope.extensionList}" var="i">
			<tr>
				<td>${i.extensionId}</td>
				<td>${i.extensionName}</td>
				<td>${i.extensionDescription}</td>
				<td>${i.ct.categoryName}
				<td>
				<div class="btn-group mb-sm"><button class="btn btn-inverse dropdown-toggle" data-toggle="dropdown" type="button" name="${i.extensionId}">Action<span class="caret"></span></button>
				<ul class="dropdown-menu" role="menu">
				<li><a href="<%=request.getContextPath()%>/ExtensionController?flag=EditExtension&extensionId=${i.extensionId}">EDIT</a></li>
				<li><a href="<%=request.getContextPath()%>/ExtensionController?flag=DeleteExtension&extensionId=${i.extensionId}">DELETE</a></li>
				</ul>
				</div>
				</td>
			</tr>
			
			<c:remove var="extensionList" scope="session" />
		</c:forEach>
              </tbody>
                  </table>
          </div>
